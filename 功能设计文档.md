...
- 自动忽略系统隐藏/临时目录（`$RECYCLE.BIN`、`System Volume Information`、`._` 开头的 macOS 隐藏文件、`.DS_Store`、`.safe-keep` 等）
...
1. **目标目录识别**：用户选择目标目录后，App 检查该目录下是否存在 `.safe-keep` 文件
   - 存在 → 读取 `folder_id`，查询 `backup_folders` 表获取历史备份记录
   - 不存在 → 全新备份，将会创建新的 `folder_id` 并写入 `.safe-keep`
2. **源端扫描**：完整遍历源端目录树，得到 `Vec<FileEntry>`（只读元数据，不做哈希）
3. **内存比对**：将扫描结果与数据库中该 `folder_id` 下的 `files` 记录做内存级比对

**对比策略（三级）：**

- **一级对比（默认）**：`relative_path + 文件大小 + 修改时间`
  - 数据库中存在且三者一致 → "已备份"
  - 数据库中存在但任一不同 → "已变更"
  - 数据库中不存在 → "新增"
  - 数据库中有但源端不存在（源端已删除）→ 标记"源端已消失"，不纳入本次备份
- **二级对比（可选）**：读取文件头部 4KB + 尾部 4KB，计算 xxHash 确认
...
- **目标端写入 `.safe-keep` 标志文件**：首次备份完成后，在目标根目录写入隐藏文件 `.safe-keep`

#### 3.1.4 数据库模块（SQLite）

**备份文件夹表（核心设计：folder_id 机制）：**

每次备份时在**目标目录**下写入一个隐藏标志文件 `.safe-keep`（约 150 字节），内容为 JSON：

```json
{ "folder_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890", "source_label": "手机DCIM" }
```

此文件的作用：
- **解耦源端身份**：无论源设备插在哪个盘符、路径如何变化，只要选择同一个目标目录，App 就能通过 `.safe-keep` 中的 `folder_id` 匹配到数据库中的历史记录
- **支持多套备份方案**：同一台手机可以备份到不同目标目录，各自有独立 `folder_id`
- **不依赖源端设备**：不需要在手机/U 盘等不可控介质上写入任何文件

```sql
CREATE TABLE IF NOT EXISTS backup_folders (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    folder_id       TEXT NOT NULL UNIQUE,       -- UUID（与 .safe-keep 文件中的一致）
    source_label    TEXT,                        -- 用户给源端起的名字（如"手机DCIM"）
    source_path     TEXT NOT NULL,               -- 首次备份时的源路径
    dest_path       TEXT NOT NULL,               -- 目标根目录路径
    total_files     INTEGER DEFAULT 0,           -- 总备份文件数
    total_bytes     INTEGER DEFAULT 0,           -- 总备份字节数
    last_backed_up_at TEXT,                      -- 最近一次备份时间
    created_at      TEXT NOT NULL
);

CREATE UNIQUE INDEX idx_backup_folders_dest ON backup_folders(dest_path);
```
...
CREATE TABLE IF NOT EXISTS files (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    folder_id       TEXT NOT NULL REFERENCES backup_folders(folder_id),
                    -- 关联到 backup_folders，实现按备份方案隔离
    source_path     TEXT NOT NULL,               -- 源文件完整路径
    relative_path   TEXT NOT NULL,               -- 相对于源目录的路径（如 "DCIM/100APPLE/IMG_0001.jpg"）
...
- 目标路径选择组件：
  - 同上
  - 显示目标盘剩余空间、已备份文件数
  - **智能识别**：选择目标后自动检测是否包含 `.safe-keep` 文件
    - 检测到 → 显示该备份任务的摘要信息（源标签、上次备份时间、文件总数）
    - 未检测到 → 显示"全新备份"
- 源标签命名（首次备份时弹出）：让用户给本次备份源起个名字（如"2024-06 日本旅行"、"手机 DCIM"），存入 `.safe-keep` 的 `source_label` 字段
- 过滤器：
  - 图片格式勾选框组（全选/取消）
  - 视频格式勾选框组（全选/取消）
  - 最小文件大小输入（默认 0）
...
│   │   │   ├── conflict.rs     # 冲突处理
│   │   │   ├── progress.rs     # 进度回调
│   │   │   └── safekeep.rs     # 备份完成后写入 .safe-keep 标志文件
│   │   ├── hasher/             # 哈希计算
│   │   │   ├── mod.rs
│   │   │   ├── fast_hash.rs    # xxHash 头尾哈希
│   │   │   └── sha256.rs       # 全文件 SHA256
│   │   ├── folder_id/          # .safe-keep 标志文件管理
│   │   │   ├── mod.rs
│   │   │   ├── read.rs         # 读取目标端的 .safe-keep
│   │   │   ├── write.rs        # 写入/更新 .safe-keep
│   │   │   └── generate.rs     # 生成 UUID + 管理 folder_id
│   │   ├── database/           # 数据库层
│   │   │   ├── mod.rs
│   │   │   ├── models.rs       # 数据模型
│   │   │   ├── migrations.rs   # 建表/迁移
