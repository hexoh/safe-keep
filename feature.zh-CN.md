...
#### 3.2.10 国际化要求（全站横切关注点）

**原则：所有面向用户的文本必须走 i18n，不允许硬编码中文/英文字符串。**

##### 技术选型

- 前端：`vue-i18n` v9+（Composition API 模式）
- 后端（Rust）：错误消息和通知文本统一使用英文枚举，由前端根据 locale 翻译展示；Rust 层不负责文本渲染

##### 翻译文件结构

```
src/locales/
├── zh-CN.json          # 简体中文（默认，与 feature.zh-CN.md 的 locale 一致）
├── en.json             # English
└── index.ts            # 导出 locale 配置、动态切换函数
```

- 使用扁平 key 结构（如 `home.source_path_label`），禁用嵌套 key
- key 命名规则：`页面/组件.具体描述`，全小写 + 下划线
- 每个 key 必须同时存在于两个语言文件中，缺少翻译时 fallback 到英文

##### 语言切换

- 设置页提供语言下拉框（中文 / English）
- 切换后立即生效，不刷新页面（`locale.value` 为响应式 ref）
- 切换后的选择持久化到 SQLite `settings` 表
- 应用启动时从 `settings` 表读取上次语言选择
- 首次启动根据 `navigator.language` 自动匹配（匹配不到默认中文）

##### 需要翻译的内容覆盖范围

| 模块 | 内容 | 说明 |
| --- | --- | --- |
| 所有页面标题、按钮、标签 | 全量文案 | 不允许硬编码 |
| 文件大小/时间格式化 | `n` / `d` 过滤器 | 使用 vue-i18n 的数字/日期格式化，按 locale 显示（如 2026/7/21 vs July 21, 2026） |
| 速度单位 | MB/s、GB/s 等 | 单位本身英文不变，但数字格式按 locale |
| 状态标签 | 新增/已备份/已变更/已删除 | 关键业务用词 |
| 设备类型显示 | MTP / U盘 / 本地磁盘 | 枚举值翻译 |
| 错误消息 | Rust 侧抛出的错误码 → 前端翻译 | Rust 返回错误码如 `ERR_DISK_FULL`，前端映射到翻译文本 |
| 进度提示 | "正在复制 xxx... ✓" | 文件名不变，模板文本翻译 |
| 确认弹窗 | 删除确认、取消确认等 | 所有弹窗文本 |
| 通知/Toast | 操作结果通知 | 成功/失败/警告消息 |
| 日志面板 | 操作日志文本 | 如"扫描完成，发现 X 个文件" |

##### 不需要翻译的内容

- 文件名/路径（保持原文）
- 技术术语（如 SHA256、xxHash、MTP、FAT32）
- 单位符号（MB/s、GB、KB）
- 版本号
- 设备序列号、UUID 等机器标识
...
- 所有 Vue 模板中使用 `$t()` 或 Composition API 的 `t()` 函数
- JavaScript/TypeScript 代码中非模板字符串使用 `t()` 函数
- 禁止在代码中出现中文/英文字面量（如 `"开始备份"`），必须通过 i18n key 引用
- 新增翻译 key 时，同步更新两个语言文件
- PR 审查时检查是否引入了硬编码字符串
...
- 所有页面完成 i18n 接入
- 中文/英文两套翻译文件完整覆盖
- 语言切换功能正常工作（设置页切换 + 持久化 + 启动恢复）
- 日期、数字按 locale 格式化正确
- 无硬编码字符串残留

---

#### 3.2.11 主题切换要求（全站横切关注点）

**原则：支持浅色/深色/跟随系统三种模式，切换后全局即时生效。**

##### 技术方案

- 使用 CSS 自定义属性（`--color-*`）定义颜色变量，所有组件引用这些变量
- 通过 `document.documentElement.setAttribute('data-theme', 'light' | 'dark')` 切换主题
- 移除 `data-theme` 属性时自动跟随系统（`prefers-color-scheme`）

##### CSS 变量层级

| 层级                                  | 说明                                                  |
| ------------------------------------- | ----------------------------------------------------- |
| `:root`                               | 基础色板（`--vt-c-white`、`--vt-c-black` 等原始色值） |
| `:root, [data-theme='light']`         | 浅色模式变量映射                                      |
| `[data-theme='dark']`                 | 深色模式变量映射                                      |
| `@media (prefers-color-scheme: dark)` | 跟随系统时的深色回退                                  |

##### 三种模式的行为

| 模式     | `data-theme` 属性    | 颜色来源                                   |
| -------- | -------------------- | ------------------------------------------ |
| 浅色     | `data-theme="light"` | 固定浅色变量                               |
| 深色     | `data-theme="dark"`  | 固定深色变量                               |
| 跟随系统 | 不设置               | `@media (prefers-color-scheme: dark)` 决定 |

##### 切换实现

- 创建 `useTheme` composable，返回响应式 `theme` ref
- 主题切换通过 `localStorage` 持久化（非 SQLite，因为 CSS 需要在应用初始化前加载）
- 设置页提供下拉框：浅色 / 深色 / 跟随系统
- 切换后立即生效，无闪烁（在 `main.ts` 入口文件最先读取 `localStorage` 设置 `data-theme`）

##### 设计规范

- 所有颜色必须通过 CSS 变量引用，不允许硬编码色值（`#fff`、`#333` 等）
- 每个新增的颜色变量必须在浅色/深色两组中都有定义
- 图标也应支持主题切换（使用 CSS filter 或 SVG fill 继承 currentColor）
- Element Plus 组件的主题通过配置 `data-theme` 属性自动适配（Element Plus 支持 CSS 变量覆盖）
...
│   │   ├── base.css            # CSS 变量（主题色板）
│   │   ├── main.css            # 全局样式入口
│   │   └── variables.module.scss  # SCSS 变量 / mixin
│   ├── assets/                 # 静态资源（图片、图标等）
│   └── locales/                # 国际化
│       ├── zh-CN.json
│       └── en.json
